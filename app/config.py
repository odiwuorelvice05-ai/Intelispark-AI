...
class Settings:
    ...
    # Agent (tool-calling) mode. ON by default so the real Mistral agent is used when its Vercel key is configured.
    agent_mode: str = os.getenv("INTELISPARK_AGENT", "on").strip().lower()      # "on" | "off"
