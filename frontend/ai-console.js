...
(function () {
  ...
  function renderIntelligence(p) {
    const examples = ['How much is the Samsung Galaxy A15?','Do you have any phones under 30000?','Which phone is best for gaming?','I want to buy the iPhone 13.'];
    p.innerHTML = head('Test Intelispark','Send a customer message through the real intelligence loop and inspect what the engine understood.','<span class="pill">LIVE ENGINE</span>') + `
      <div class="ai-console"><section class="card ai-chat-card"><div class="section-kicker">CUSTOMER SIMULATOR</div><h3>Ask as a customer</h3><p class="ai-muted">The request is evaluated against this shop's real Supabase catalog.</p><div class="ai-examples">${examples.map(x => `<button class="ghost ai-example" type="button">${esc(x)}</button>`).join('')}</div><form id="aiTestForm" class="ai-form"><textarea id="aiMessage" rows="5" required placeholder="e.g. Do you have a Samsung phone under KES 30,000 with a good camera?"></textarea><div class="form-actions"><span class="ai-hint">Answers use only this shop's catalog and profile.</span><button class="primary" id="aiSend" type="submit">Run intelligence →</button></div></form><div id="aiReply" class="ai-result" hidden></div></section><aside class="card ai-inspector"><div class="section-kicker">ENGINE INSPECTOR</div><h3>What Intelispark sees</h3><div id="aiInspector" class="ai-inspector-body"><div class="empty">Run a message to inspect intent, entities, sales signals and catalog matches.</div></div></aside></div>`;
    document.querySelectorAll('.ai-example').forEach(btn => btn.addEventListener('click', () => { $('#aiMessage').value = btn.textContent; $('#aiMessage').focus(); }));
  ...
  async function runIntelligence(ev) {
    ...
    button.disabled=true; button.textContent='Thinking…'; $('#aiReply').hidden=false; $('#aiReply').innerHTML='<div class="ai-loading">Retrieving business knowledge and reasoning over the request…</div>';
    try {
    ...
    finally { button.disabled=false; button.textContent='Run intelligence →'; }
  }
